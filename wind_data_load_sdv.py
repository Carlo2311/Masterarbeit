...
samples_total = [50,100,200,300,600,900,1500,3000,4500,6000,9000]
...
for iter in range(iteration):
    for s_i, samples_tot in enumerate(samples_total):
        ...
        # plt.figure()
        # plt.errorbar(windspeed[:300], root_myb_mean[:300], yerr=root_myb_sdv[:300], fmt='o', capsize=5, color='g')
        ...
        # plt.figure()
        # plt.errorbar(windspeed[:300], root_myb_mean_normalized[:300], yerr=root_myb_sdv_normalized[:300], fmt='o', capsize=5, color='g')
        # plt.xlabel('windspeed [m/s]')
        # plt.ylabel('blade load [mm/s^2]')
        # plt.grid()
        # plt.show()
        ...
        samples_y_resized_all = np.reshape(samples_y_all, (30, 300))

        ###
        samples_y_resized = samples_y_resized_all[index_rep,:]
        samples_y_resized = samples_y_resized[:,index_samples]
        samples_y_mean = samples_y_resized.reshape(samples_tot)

        samples_y_sdv_all = root_myb_sdv_normalized[:9000]
        samples_y_sdv_resized_all = np.reshape(samples_y_sdv_all, (30, 300))
        samples_y_sdv_resized = samples_y_sdv_resized_all[index_rep,:]
        samples_y_sdv_resized = samples_y_sdv_resized[:,index_samples]
        samples_y_sdv = samples_y_sdv_resized.reshape(samples_tot)

        # plt.figure()
        # plt.errorbar(samples_x[0,:300], samples_y[:300], yerr=samples_y_sdv[:300], fmt='o', capsize=5, color='g')
        # plt.xlabel('windspeed [m/s]')
        # plt.ylabel('blade load [mm/s^2]')
        # plt.grid()
        # plt.show()

        samples_y = np.random.normal(samples_y_mean[:, np.newaxis], samples_y_sdv[:, np.newaxis], (samples_y_mean.shape[0], 1)).flatten()

        # plt.figure()
        # # plt.errorbar(samples_x[0,:300], samples_y_mean[:300], yerr=samples_y_sdv[:300], fmt='o', capsize=5, color='g')
        # plt.scatter(samples_x[0,:], samples_y[:], s=5)
        # plt.show()
        

        samples_y_test = root_myb_mean_normalized[9000:]
        samples_y_test_resized = np.reshape(samples_y_test, (30, 30))
        samples_y_sdv_test = root_myb_sdv_normalized[9000:]
        samples_y_sdv_test_resized = np.reshape(samples_y_sdv_test, (30, 30))
        ...
        # plt.figure()
        # plt.scatter(samples_x[0,:300], samples_y[:300], s=5)
        # plt.scatter(samples_x_test[0,:30], samples_y_test[:30], s=5)
        # plt.xlabel('windspeed [m/s]')
        # plt.ylabel('blade load [mm/s^2]')
        # plt.grid()
        # # tikzplotlib.save(rf"tex_files\wind_data\ouput_samples_1.tex")
        # plt.figure()
        # plt.scatter(samples_x[1,:300], samples_y[:300], s=5)
        # plt.scatter(samples_x_test[1,:30], samples_y_test[:30], s=5)
        # plt.xlabel('turbulence intensity')
        # plt.ylabel('blade load [mm/s^2]')
        # plt.grid()
        # # tikzplotlib.save(rf"tex_files\wind_data\ouput_samples_2.tex")
        # plt.figure()
        # plt.scatter(samples_x[2,:300], samples_y[:300], s=5)
        # plt.scatter(samples_x_test[2,:30], samples_y_test[:30], s=5)
        # plt.xlabel('rho')
        # plt.ylabel('blade load [mm/s^2]')
        # plt.grid()
        # # tikzplotlib.save(rf"tex_files\wind_data\ouput_samples_3.tex")
        # plt.figure()
        # plt.scatter(samples_x[3,:300], samples_y[:300], s=5)
        # plt.scatter(samples_x_test[3,:30], samples_y_test[:30], s=5)
        # plt.xlabel('yaw angle')
        # plt.ylabel('blade load [mm/s^2]')
        # plt.grid()
        # # tikzplotlib.save(rf"tex_files\wind_data\ouput_samples_4.tex")
        # plt.show()
        ...
        samples_pce_std_y = np.mean(samples_y_sdv_resized, axis=0)
        ...
        # std_sim = np.std(samples_y_sdv_test_resized, axis=0)
        std_sim = np.mean(samples_y_sdv_test_resized, axis=0)
        samples_sim_all = np.random.normal(mean_sim[:, np.newaxis], std_sim[:, np.newaxis], (mean_sim.shape[0], 10000))
        error_spce[iter,s_i] = spce.compute_error(dist_spce[:size_y_test,:], samples_sim_all)
        error_pce[iter,s_i] = spce.compute_error(dist_pce[:size_y_test,:], samples_sim_all)
        # error_gpr[iter,s_i] = spce.compute_error(dist_gpr[:size_y_test,:], samples_sim_all)
        print('spce wasserstein distance2 = ', error_spce)
        print('pce wasserstein distance2 = ', error_pce)
        print('gpr wasserstein distance2 = ', error_gpr)
        ...
        y_samples_plot = samples_sim_all[x_index_plot,:]
        ...
        # #    test_statistic_gpr[i], p_value_gpr[i] = stats.ks_2samp(gpr_dist_ks[i,:], samples_y_test_resized[:,i])
            ...
            test_statistic_spce[i], p_value_spce[i] = stats.ks_2samp(spce_dist_ks[i,:], samples_sim_all[i,:])
            test_statistic_pce[i], p_value_pce[i] = stats.ks_2samp(pce_dist_ks[i,:], samples_sim_all[i,:])
            # test_statistic_gpr[i], p_value_gpr[i] = stats.ks_2samp(gpr_dist_ks[i,:], samples_sim_all[i,:])
...
np.save(f'C:/Users/carlo/Masterarbeit/Masterarbeit/solutions_wind/load/convergence_samples/sdv/samples_total_{samples_total[0]}_{samples_total[-1]}.npy', samples_total)
np.save(f'C:/Users/carlo/Masterarbeit/Masterarbeit/solutions_wind/load/convergence_samples/sdv/error_spce_{samples_total[0]}_{samples_total[-1]}.npy', error_spce)
np.save(f'C:/Users/carlo/Masterarbeit/Masterarbeit/solutions_wind/load/convergence_samples/sdv/error_pce_{samples_total[0]}_{samples_total[-1]}.npy', error_pce)
...
np.save(f'C:/Users/carlo/Masterarbeit/Masterarbeit/solutions_wind/load/convergence_samples/sdv/nrmse_spce_{samples_total[0]}_{samples_total[-1]}.npy', nrmse_spce)
np.save(f'C:/Users/carlo/Masterarbeit/Masterarbeit/solutions_wind/load/convergence_samples/sdv/nrmse_pce_{samples_total[0]}_{samples_total[-1]}.npy', nrmse_pce)
...
np.save(f'C:/Users/carlo/Masterarbeit/Masterarbeit/solutions_wind/load/convergence_samples/sdv/p_value_spce_{samples_total[0]}_{samples_total[-1]}.npy', p_value_spce_mean)
np.save(f'C:/Users/carlo/Masterarbeit/Masterarbeit/solutions_wind/load/convergence_samples/sdv/p_value_pce_{samples_total[0]}_{samples_total[-1]}.npy', p_value_pce_mean)
...
np.save(f'C:/Users/carlo/Masterarbeit/Masterarbeit/solutions_wind/load/convergence_samples/sdv/test_statistic_spce_{samples_total[0]}_{samples_total[-1]}.npy', test_statistic_spce_mean)
np.save(f'C:/Users/carlo/Masterarbeit/Masterarbeit/solutions_wind/load/convergence_samples/sdv/test_statistic_pce_{samples_total[0]}_{samples_total[-1]}.npy', test_statistic_pce_mean)
